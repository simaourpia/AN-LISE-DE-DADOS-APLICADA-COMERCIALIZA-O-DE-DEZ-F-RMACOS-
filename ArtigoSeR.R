...
# Lista de pacotes utilizados no código
pacotes <- c("writexl", "ggridges", "forcats", "ggplot2", "dplyr", "tidyr", "psych", "gganimate", "scales", 
             "janitor", "tidyverse", "ggthemes", "gridExtra", "dlookr", "prettydoc", "dataMaid", "rcompanion", 
             "data.table", "ggpattern", "readxl")

# Função para citar pacotes
citar_pacotes <- function(pacotes) {
  for (pacote in pacotes) {
    print(citation(pacote))
  }
  # Citar o próprio R
  print(citation())
}
citation("stringr")
# Chamar a função para citar os pacotes
citar_pacotes(pacotes)
